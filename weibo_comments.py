...
def weibo():
    ...
    with open('weibo_comments.txt', 'w', encoding='gb18030') as f:
        for speed in range(20):
            ...
            comments=browser.find_elements(By.XPATH,"//div[@class='vue-recycle-scroller__item-wrapper']/div")
            dit_list=len(comments)
            for txt in range(dit_list):
                test=browser.find_element(By.XPATH,"//div[@class='vue-recycle-scroller__item-wrapper']/div[{}]".format(txt+1)).text
                print(test)
                txt_comments.add(test)
